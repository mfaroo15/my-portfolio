...
export default function WorkPage() {
  return <main id="main-content">
    ...
    <section className="section work-index"><div className="container">
      {work.map((item, index) => <Link className="work-index-row" href={`/work/${item.slug}`} key={item.slug}>
        <span>{String(index + 1).padStart(2, "0")}</span><div className="work-index-logo"><Image src={item.logo} alt={item.name} width={420} height={120} /></div><div><p>{item.sector}</p><h2>{item.title}</h2><small>{item.summary}</small></div><ArrowUpRight aria-hidden="true" />
      </Link>)}
    </div></section>
    <ContactBand title="Build the next case study with us." />
  </main>;
}
