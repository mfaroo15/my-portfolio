  ...
  return <main id="main-content">
    ...
    <section className="section work-index"><div className="container">
      {work.map((item, index) => <article className="work-index-row" key={item.slug}>
        <span>{String(index + 1).padStart(2, "0")}</span><div className="work-index-logo"><Image src={item.logo} alt={item.name} width={420} height={120} /></div><div><p>{item.sector}</p><h2><Link href={`/work/${item.slug}`}>{item.title}</Link></h2><small>{item.summary}</small><div className="work-index-actions"><Link href={`/work/${item.slug}`}>View case study</Link><a href={item.website} target="_blank" rel="noopener noreferrer">Visit website <ArrowUpRight aria-hidden="true" size={15} /></a></div></div>
      </article>)}
    </div></section>
