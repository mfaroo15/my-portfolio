...
export default function Home() {
  ...
  return <main id="main-content"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData).replace(/</g, "\\u003c") }} />
    ...
    <section className="section industry-section"><div className="container section-top"><div><p className="kicker">INDUSTRY EXPERIENCE &amp; APPLICATIONS</p><h2>Experience across operating environments.</h2></div><p>Our operating-business work is concentrated in construction, infrastructure and international trade. Broader references reflect team experience and where our technology capabilities apply.</p></div><div className="container industry-experience-groups"><div className="industry-group"><div className="industry-group-heading"><span>01</span><h3>Demonstrated operating experience</h3></div><div className="industry-list">{industries.filter((item) => item.basis === "operating").map((item) => <article key={item.title}><span>{item.code}</span><h3>{item.title}</h3><p>{item.text}</p></article>)}</div></div><div className="industry-group"><div className="industry-group-heading"><span>02</span><h3>Team experience &amp; technology applications</h3></div><div className="industry-list">{industries.filter((item) => item.basis === "team").slice(0, 4).map((item) => <article key={item.title}><span>{item.code}</span><h3>{item.title}</h3><p>{item.text}</p></article>)}</div></div><Link className="text-link" href="/industries">Explore industry experience <ArrowRight aria-hidden="true" size={16} /></Link></div></section>
